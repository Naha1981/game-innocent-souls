...
import { markGamePaid } from '../../../../../lib/db/game-records';
...
export async function POST(request: Request) {
  ...
  if (!merchantId || !passphrase || !process.env.DATABASE_URL?.trim()) return new NextResponse('Not configured', { status: 503 });
  ...
  try {
    ...
    if (!suppliedSignature || signatureForItn(fields, passphrase) !== suppliedSignature) return new NextResponse('Invalid signature', { status: 400 });
    ...
    const validationUrl = process.env.PAYFAST_SANDBOX === 'true' ? 'https://sandbox.payfast.co.za/eng/query/validate' : 'https://www.payfast.co.za/eng/query/validate';
    const validationResponse = await fetch(validationUrl, { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: raw, cache: 'no-store' });
    ...
    console.info(JSON.stringify({ type: 'nahakids.payment.completed', paymentId, package: packageId, jobId: order.jobId, pfPaymentId: params.get('pf_payment_id'), amount: gross }));
